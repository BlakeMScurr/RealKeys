<script lang="ts">
    import { AbstractNote, NewNote } from "./../lib/music/theory/notes"
    ...
    function handleInput () {
        if (new AbstractNote(input).string() == expected.abstract.string()) {
            update()
            draw(canvas)
        } else {
            console.log("expected", expected.abstract.string(), "got", input)
        }
    }
...
String {stringNote.abstract.string()}, Fret {fret}
